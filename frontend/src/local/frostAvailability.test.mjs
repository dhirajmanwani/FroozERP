...
test("a desktop FroozERP reads FROST from this machine whatever the cloud is doing", () => {
  for (const apiUrl of ["http://localhost:5051", "http://127.0.0.1:5051", "http://[::1]:5051/"]) {
    assert.equal(resolveFrostDataSource({ apiUrl, cloudApiMode: false }), "local");
    assert.equal(resolveFrostDataSource({ apiUrl, cloudApiMode: true }), "local");
  }
  assert.equal(resolveFrostDataSource({ apiUrl: "https://froozerp.example.com", cloudApiMode: true }), "cloud");
});

test("a local FROST loads while the cloud is down, because none of its data is in the cloud", () => {
  ...
  assert.equal(decision.shouldLoad, true);
...
test("App.jsx asks this module instead of gating FROST on the cloud", () => {
  assert.match(appSource, /resolveFrostLoadDecision\(/);
  assert.match(appSource, /resolveFrostProviderOptions\(/);
  // The old gate refused the whole load whenever the cloud was unreachable, whatever FROST was
  // reading. If it comes back, every FROST panel goes dark again during a cloud outage.
  assert.doesNotMatch(appSource, /if \(!runtimeConnectivity\.internetAvailable \|\| cloudHealth\?\.online === false\) \{/);
});
